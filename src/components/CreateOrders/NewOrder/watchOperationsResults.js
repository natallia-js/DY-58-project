...
export const useWatchOperationsResults = (inputVals) => {
  ...
  watch(() => store.getters.getDispatchOrderResult, (newVal) => {
    ...
    if (!newVal.error) {
      state.orderDispatchedSuccessfully = true;

      // Если не установить данный флаг, приложение перед переходом к закладке издания распоряжения/уведомления ЭЦД
      // спросит пользователя, действительно ли тот хочет покинуть страницу
      ...
      // Если издан любой документ, кроме приказа ЭЦД с особой отметкой "ТУ", то после успешного окончания
      // процесса издания документа необходимо осуществить переход на главную страницу приложения.
      // В случае приказа ЭЦД с особой отметкой "ТУ" необходимо остаться на текущей странице и инициировать издание
      // связанного с изданным приказом распоряжения/уведомления ЭЦД.
