<template>
  <div>
    ...
    <div class="dy58-help-definition">
      <p class="dy58-help-text">
        ...
        <ol class="dy58-help-list">
          ...
          <li>
            <span>
              в рамках цепочки распоряжений все заявки считаются действующими при условии, что
              в цепочке документов за ними (не обязательно сразу) не следует уведомление;
            </span>
          </li>
          <li>
            <span>
    ...
    <p class="dy58-help-paragraph">
      ...
      поле, высоту которого можно менять вручную (мышью, с помощью правого нижнего угла поля). Текст
      "бесшаблонного" документа может редактироваться произвольным образом. Все символы, в том числе переносы строк,
      будут сохранены и отображены в дальнейшем.
    </p>
    <p class="dy58-help-paragraph">
    ...
    <p class="dy58-help-paragraph">
      ...
      Адресаты из числа "Иных адресатов" не получат экземпляр созданного документа, т.к. системой ДУ-58 не предусмотрены
    ...
    <p class="dy58-help-paragraph">
      ...
      (при условии отсутствия влияния на документ "внешних" факторов: создание на основе "окна", черновика, другого документа ... -
      см. таблицы 2 и 4).
      ...
      Если таблица ДСП делится на поездные участки, то действие данных кнопок распространяется только на выбранные поездные участки
      либо на все участки, если ни один из них не выбран.
    </p>
    <p class="dy58-help-paragraph">
    ...
    <br />
    <p class="dy58-help-paragraph">
      Для очистки всех таблиц адресатов предназначена кнопка
      <Button
        icon="pi pi-times"
        class="p-button-secondary p-button-sm dy58-order-action-button p-m-1"
      />, расположенная рядом с наименованием поля адресатов документа.
    </p>
    <p class="dy58-help-paragraph">
      Рассмотрим теперь, как влияют действия, производимые пользователем, на изменение содержимого полей
      формы создания нового документа. Соответствующая информация представлена в таблице 4.
    </p>
    <br />
    <DataTable
