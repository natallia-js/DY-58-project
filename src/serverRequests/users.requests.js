...
// На участках ДНЦ нас интересуют только пользователи с полномочиями DNC_FULL и REVISOR
  ...
  const response = await makeServerRequest({
    ...
    params: {
      ...
      credsGroups: [{
        ...
        creds: [APP_CREDENTIALS.DNC_FULL, APP_CREDENTIALS.REVISOR],
      }],
    },
  });
...
// На станциях нас интересуют только пользователи с полномочиями DSP_FULL, DSP_Operator, STATION_WORKS_MANAGER и REVISOR
export const getStationsWorkPoligonsUsers = async ({ stationIds, onlyOnline, includeWorkPlaces }) => {
  const response = await makeServerRequest({
    ...
    params: {
      ...
      credsGroups: [{
        credsGroup: DY58_CREDENTIALS_GROUP_NAME,
        creds: [APP_CREDENTIALS.DSP_FULL, APP_CREDENTIALS.DSP_Operator, APP_CREDENTIALS.STATION_WORKS_MANAGER, APP_CREDENTIALS.REVISOR],
      }],
      includeWorkPlaces,
    },
  });
...
// На участках ЭЦД нас интересуют только пользователи с полномочиями ECD_FULL и REVISOR
export const getECDSectorsWorkPoligonsUsers = async ({ sectorIds, onlyOnline }) => {
  const response = await makeServerRequest({
    ...
    params: {
      ...
      credsGroups: [{
        credsGroup: DY58_CREDENTIALS_GROUP_NAME,
        creds: [APP_CREDENTIALS.ECD_FULL, APP_CREDENTIALS.REVISOR],
      }],
    },
  });
  return response.data;
